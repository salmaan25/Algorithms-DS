...
int n, k;
void find_prod(vi& ans, ll lo, ll hi, vi& a) {
    if(lo == hi) {
        // for(ll i = 0; i < 10; i++)
        //     ans.pb(a[i]);
        for(int i = 0; i <= a[lo]; i++)
            ans.pb(1); 
        return;
    }
    ll mid = (lo+hi)/2;
    vi ans1, ans2;
    find_prod(ans1, lo, mid, a);
    find_prod(ans2, mid+1, hi, a);
    ans = FFT::mod_multiply(ans1, ans2, MOD, true);
}
...
int32_t main() {
    ...
    // }
    vi ans;
    find_prod(ans, 0, n-1, a);
    ll val = 0;
    // for(ll i = 0; i < (ll)ans.size(); i++)
    //     val = (val + ans[i]*ans[i])%MOD;
    // cout<<val%MOD<<endl;
    cout<<ans[k]<<endl;
}